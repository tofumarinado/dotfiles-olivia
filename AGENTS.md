...
They are managed with **GNU Stow**: every top-level directory is a Stow package whose internal tree mirrors `$HOME`, so running `stow --target=$HOME <package>` creates the right symlinks.
...
Stow target is `$HOME` for both — `config` symlinks into `~/.config`, `themes` into `~/.themes`.
...
### GTK Theme (`themes/.themes/diinki-retro-dark/`)
...
- **Stow a new package:** create `config/<name>/...` mirroring `~/.config/<name>/`, then `stow --target=$HOME/.config config/<name>`.
- **Apply all configs:** `stow --target=$HOME config themes`
